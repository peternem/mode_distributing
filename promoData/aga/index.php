...
<body>
<body>
<h3 class="h3TabContent">AGA Promotional Documents</h3>
		...
		foreach ($ffs as $ff) {
			if (is_array($exclude) and !in_array($ff, $exclude)) {
				if ($ff != '.' && $ff != '..' && $ff != 'thumbs' && $ff != 'images' && $ff != 'pdf' && $ff != 'pdfimage') {
					...
					if (!is_dir($dir . '/' . $ff)) {
						echo '<li class=\'boxListItem\'><a href="promoData/aga/' . ltrim($dir . '/' . $ff, './') . '" title="'.pathinfo($ff, PATHINFO_FILENAME).'" target="_blank">';
						echo '<img src=\'promoData/aga/pdfimage/' .pathinfo($ff, PATHINFO_FILENAME). '.jpg\' class=\'box\'/></a>';
						echo '<a href="promoData/aga/' . ltrim($dir . '/' . $ff, './') . '">' . strtoupper(substr($ff1, 0, 20)) . '</a>';
					} else {
